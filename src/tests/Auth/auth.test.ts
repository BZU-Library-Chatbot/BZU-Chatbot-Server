...
describe("POST /auth/login && POST /auth/signup", () => {
  let variables: any = {};
  it.each([
...
describe("PATCH /auth/sendCode", () => {
  it.each([
    ...
    const response = await request(app).patch("/auth/sendCode").send({ email });
    // .expect(expected);
...
describe("PATCH /auth/activate/:adminId", () => {
  ...
  it("", async () => {
    await request(app).patch(`/auth/activate/${variables.adminId}`).expect(400);
  });
...
describe("GET /admin", () => {
  ...
  it.each([
    ...
    [400, -1, 1, "Active"],
    [400, 10, -1, "Active"],
    [400, "invalid", 1, "Active"],
    [400, 10, "invalid", "Active"],
    // Edge cases
    [200, 1, 1, "Active"],
    [200, 100, 1, "Active"],
    [200, 10, 1, "Not Active"],
