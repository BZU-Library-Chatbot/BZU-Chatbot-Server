...
export const signup= async (req,res,next)=>{
  ...
  //             <tr>
  //               <td align="left" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
  //                 <p style="margin: 0;">If that doesn't work, click <a href="${rLink}">here</a> to resend email.</p>
  ...
  //                 <p style="margin: 0;">Cheers,<br> Tariq :D</p>
  //               </td>
  //             </tr>
  //             <!-- end copy -->
  ...
  //   const createdUser = await userModel.create({userName,email,password:hashPassword});
  //   return res.status(201).json({message:"success",user:createdUser._id});
    ...
    const html = `<a href="${link}">Verify your email</a> <br /> <br /> <a href="${Rlink}">Send new email</a>`;
    await sendEmail(email,'Confirm Email',html);
...
export const confirmEmail = async(req,res, next)=>{
    ...
    // return res.json({count:user.modifiedCount == true})

    
    if(!user?.modifiedCount){
      return next(new Error('no user to confirm'), {cause:404});
    }
    // return res.status(200).json({message:"email has been confirmed"});
